...
export class VisualEngine {
  ...
  private mappingSignature=''; private previousFrame=performance.now(); private jankSamples=0;
  private renderFailed=false;
  private failedMediaId?:string; private selectedMediaId?:string;
  ...
  apply(snapshot: OutputSnapshot) {
    this.snapshot=snapshot; const p=snapshot.project; const selected=p.assets.find(a=>a.id===p.activeMediaId);
    if(selected&&selected.id!==this.selectedMediaId){this.selectedMediaId=selected.id;const instant=!this.material.uniforms.uHasMedia.value;void this.media.select(selected,instant).then(()=>{this.material.uniforms.uHasMedia.value=1;this.failedMediaId=undefined;}).catch(error=>{if(this.failedMediaId!==selected.id){this.failedMediaId=selected.id;this.onError?.(error instanceof Error?error:new Error(String(error)));}});}
    else if(!selected&&this.selectedMediaId!==undefined){this.selectedMediaId=undefined;this.media.clear();this.material.uniforms.uHasMedia.value=0;}
    const desired=p.mapping.previewMode==='eco'?.25:p.mapping.previewMode==='standard'?.5:p.mapping.previewMode==='high'?.75:p.mapping.previewMode==='1080p'?1:p.mapping.previewScale;if(!this.projector&&this.qualityScale!==desired){this.qualityScale=desired;this.resize();}const mappingSignature=JSON.stringify([p.mapping.controlMode,p.mapping.quad,p.mapping.grid]);if(mappingSignature!==this.mappingSignature){this.mappingSignature=mappingSignature;this.setMapping(p.mapping);}this.material.uniforms.uBrightness.value=p.mapping.brightness; this.material.uniforms.uBlackout.value=p.mapping.blackout?1:0;
  ...
  private loop(now:number){this.raf=requestAnimationFrame(this.loop);if(this.renderFailed)return;try{const gap=now-this.previousFrame;this.previousFrame=now;if(this.projector&&gap>75)this.jankSamples++;const host=this.canvas.parentElement??this.canvas;if(host.clientWidth!==this.lastWidth||host.clientHeight!==this.lastHeight)this.resize();this.media.update(now);this.material.uniforms.uMediaA.value=this.media.textureA;this.material.uniforms.uMediaB.value=this.media.textureB;this.material.uniforms.uTransition.value=this.media.transition;this.material.uniforms.uMediaSize.value.set(...this.media.size);this.material.uniforms.uTime.value=now*.001;const renderSize=this.renderer.getDrawingBufferSize(new THREE.Vector2());this.particles.update(now,this.media,renderSize.x,renderSize.y);this.composer.render();this.frameCount++;if(now-this.fpsStarted>=1000){const fps=this.frameCount*1000/(now-this.fpsStarted);const frameMs=(now-this.last)/Math.max(1,this.frameCount);if(this.projector){this.lowFpsSamples=fps<26||this.jankSamples>=2?this.lowFpsSamples+1:0;this.highFpsSamples=fps>29&&this.jankSamples===0?this.highFpsSamples+1:0;if(this.lowFpsSamples>=2&&this.qualityScale>.5){this.qualityScale=this.qualityScale===1?.75:.5;this.lowFpsSamples=0;this.highFpsSamples=0;this.resize();}else if(this.highFpsSamples>=12&&this.qualityScale<1){this.qualityScale=this.qualityScale===.5?.75:1;this.highFpsSamples=0;this.resize();}}const quality=this.qualityScale===1?'high':this.qualityScale===.75?'medium':'low';this.onPerformance?.({fps,frameMs,quality,serialConnected:false,globalState:'running'});this.frameCount=0;this.jankSamples=0;this.fpsStarted=now;this.last=now;}}catch(error){this.renderFailed=true;this.onError?.(error instanceof Error?error:new Error(String(error)));}}
}
