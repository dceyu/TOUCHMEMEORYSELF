import { defaultTouch, touchConfigSchema, type TouchConfig } from './touch';

// No external Arduino library. Fixed buffers and bounded measurement loops on Uno R3.
export function touchFirmware(settings:TouchConfig=defaultTouch()):string {
  const c=touchConfigSchema.parse(settings);
  return String.raw`// CENTOPIA capacitive touch v1 / Uno R3 ATmega328P / 115200 baud
...
Setting settings[8]={${c.channels.map(v=>`{${v.baseline},${v.on},${v.off},${v.pressMs},${v.releaseMs}}`).join(',')}};
bool finalTouch=${c.finalMode==='touch'?'true':'false'};
...
byte measurementSamples=${c.measurement.samples};unsigned int measurementLimit=${c.measurement.limit};
