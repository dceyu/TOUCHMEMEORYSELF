...
const schema=z.object({
  ...
  cues:z.array(z.object({kind:z.string(),label:z.string(),durationMs:finite.min(100).max(30000),intensity:finite.min(0).max(2)})).length(8).optional(),
  ...
  quantum:z.object({pointSize:finite.min(.1).max(10)}).optional(),
