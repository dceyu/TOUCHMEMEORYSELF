  ...
  return <div className="mapping-tools" style={{overflow:'auto'}}>
    ...
    <div style={{display:'grid',gap:10}}>{a.channels.map((c,i)=>{
      ...
      return <div key={i} style={{display:'grid',gridTemplateColumns:'1fr 24px 1fr 24px 1.4fr',alignItems:'center',gap:8,border:'1px solid #343d30',padding:12,borderRadius:8}}>
        ...
        <span>→</span><div><select disabled aria-label={`CH ${i+1} effect`} value={project.cues[i].kind} onChange={e=>onProject({...project,cues:project.cues.map((cue,j)=>j===i?{...cue,kind:e.target.value as CueKind,label:cueLabel(e.target.value as CueKind,project.ui.language)}:cue)})}>{kinds.map(kind=><option key={kind} value={kind}>{cueLabel(kind,project.ui.language)}</option>)}</select><button onClick={()=>onTrigger(i)}>{t('测试一次','TEST CUE')}</button><label>{t('时长 ms','DURATION ms')}<input type="number" min="300" max="10000" step="100" value={project.cues[i].durationMs} onChange={e=>onProject({...project,cues:project.cues.map((cue,j)=>j===i?{...cue,durationMs:Math.max(300,Math.min(10000,+e.target.value))}:cue)})}/></label></div>
      </div>;
    })}</div>
